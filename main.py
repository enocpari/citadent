"""
Sistema de Gestión de Clínica Dental - CitaDent

Aplicación CLI (Command Line Interface) para gestionar una clínica dental.
Permite la administración de pacientes, dentistas, citas y horarios de atención.

Funcionalidades:
- Gestión de Pacientes: agregar, buscar, actualizar, eliminar
- Gestión de Dentistas: agregar, buscar, actualizar, eliminar
- Gestión de Citas: agendar, completar, cancelar, eliminar
- Chatbot para pacientes: asistencia virtual y agendado automático
- Sistema de horarios de atención con descanso

Usage:
    python3 main.py

Menu Principal:
    1. Administrador - Acceso completo al sistema (clave: admin123)
    2. Paciente - Chatbot interactivo para consultas y citas
    3. Salir
...
# ----------------------------------------------------------------------
# Variables globales - Instancias de servicios
# ----------------------------------------------------------------------
# Servicios para manejar los datos en memoria
...
def input_con_cancelar(prompt):
    """
    Solicita input al usuario con opción de cancelar.
    
    Retorna None si el usuario ingresa '0', 'cancelar', 'atras', 'q' o 'quit'.
    Retorna el valor ingresado en caso contrario.
    
    Args:
        prompt: Mensaje a mostrar al usuario.
    
    Returns:
        str o None: El valor ingresado o None si cancela.
    """
...
def confirmar_accion(mensaje):
    ...
    Returns:
        bool: True si confirma con 's' o 'S', False en otro caso.
    """
    while True:
...
def obtener_opcion_menu(titulo, opciones):
    ...
    Args:
        ...
        opciones: Dict de {clave: descripcion} o lista de tuplas (clave, descripcion).
    
    Returns:
        str: La opción seleccionada o None si ingresa 0 para cancelar.
    """
    print(f"\n=== {titulo} ===")
...
def obtener_hora_12h():
    """
    Solicita al usuario hora y minutos (en una sola entrada) y periodo (AM/PM),
    luego retorna un objeto time.
    
    Convierte el formato 12h (1-12 con AM/PM) al formato 24h para almacenamiento
    interno. Este formato es mas intuitivo para usuarios en Bolivia.
    
    Returns:
        time: Objeto time con la hora convertida a formato 24h, o None si cancela.
    """
    while True:
        try:
...
def agregar_pacientes(instancia):
    """
    Agrega un nuevo paciente con opciones de cancelación y confirmación.
    
    Args:
        instancia: Instancia del servicio de pacientes.
    """
    print("\n--- Agregar Paciente ---")
...
def agregar_cita(instancia, servicio_horarios):
    ...
    Solicita al usuario los datos del paciente (nombre, apellidos, contacto),
    la fecha y hora de la cita. Valida que los formatos de fecha y hora sean
    correctos, que este dentro del horario de atencion, y que no haya
    otra cita pendiente en ese horario.
    
    Args:
        instancia: Instancia del servicio de citas donde se guardara la nueva cita.
        servicio_horarios: Instancia del servicio de horarios de atencion.
    """
    print("\n--- Agendar Cita ---")
    ...
    # Bucle para validar el formato de fecha (YYYY-MM-DD)
    # Se repite hasta que el usuario ingrese un formato valido
    while True:
        fecha_str = input_con_cancelar("Ingrese la fecha (YYYY-MM-DD)")
        if fecha_str is None:
            print("Operación cancelada.")
            return
        # Valida el patron: 4 digitos, guion, 2 digitos, guion, 2 digitos
        if re.match(r'^\d{4}-\d{2}-\d{2}$', fecha_str):
            try:
                # Intenta convertir la cadena a objeto date
                fecha = datetime.strptime(fecha_str, "%Y-%m-%d").date()
                break  # Sale del bucle si la conversion fue exitosa
            except ValueError:
                # Error si la fecha no existe (ej: 2026-02-30)
                print("Error: La fecha no es valida. Verifique el dia y mes.")
        else:
            # Error si el formato no coincide con YYYY-MM-DD
            print("Error: Formato invalido. Use YYYY-MM-DD (ej: 2026-04-16)")
    
    # Bucle para validar la hora con formato 12h AM/PM, horario de atencion y disponibilidad
    # Se repite hasta que el usuario ingrese un formato valido, dentro del horario de atencion
    # y sin conflictos con otras citas
    while True:
        ...
        # Verificar si hay horarios de atencion configurados
        ...
        # Verificar si la hora esta dentro del horario de atencion
        valido, mensaje = servicio_horarios.verificar_horario_atencion(fecha, hora)
        if not valido:
            print(f"Error: {mensaje}")
            continue
        
        # Verifica si el horario ya esta ocupado por otra cita pendiente
...
def mostrar_citas_completadas(instancia):
    """
    Muestra todas las citas completadas en el sistema.
    
    Utiliza el metodo get_citas_completadas() del servicio de citas
    para obtener todas las citas con estado "completada".
    
    Args:
        instancia: Instancia del servicio de citas.
    """
...
def mostrar_citas_canceladas(instancia):
    """
    Muestra todas las citas canceladas en el sistema.
    
    Utiliza el metodo get_citas_canceladas() del servicio de citas
    para obtener todas las citas con estado "cancelada".
...
def buscar_por_fecha_interactivo(instancia):
    """
    Busca citas por una fecha especifica ingresada por el usuario.
    
    Solicita la fecha en formato YYYY-MM-DD, valida el formato y luego
    busca todas las citas existentes para esa fecha.
    
    Args:
        instancia: Instancia del servicio de citas para realizar la busqueda.
    """
    # Bucle para validar el formato de fecha (YYYY-MM-DD)
    # Se repite hasta que el usuario ingrese un formato valido
    while True:
    ...
    # Busca las citas para la fecha ingresada
...
def seleccionar_cita_interactiva(instancia):
    """
    Permite buscar una cita por diferentes criterios y seleccionarla.
    
    El usuario puede buscar por ID, nombre, apellido o contacto.
    Si hay múltiples resultados, se muestra una lista para seleccionar.
    
    Args:
        instancia: Instancia del servicio de citas para realizar la busqueda.
    
    Returns:
        tuple: (cita, id) si se encontro y selecciono una cita, (None, None) si no.
    """
    print("\n--- Buscar Cita ---")
    ...
    # Si se ingreso un ID, buscar por ID primero
    if id_input:
        try:
            id_int = int(id_input)
    ...
    # Si hay multiples citas, mostrar lista para seleccionar
...
def completar_cita(instancia, servicio_pacientes):
    """
    Completa una cita seleccionada por el usuario.
    
    Permite buscar la cita por ID, nombre, apellido o contacto.
    Si la cita existe y esta pendiente, la marca como completada
    y registra al paciente en el sistema.
    
    Args:
        instancia: Instancia del servicio de citas.
        servicio_pacientes: Instancia del servicio de pacientes para registrar.
    """
    ...
    # Verificar que la cita este pendiente
...
def cancelar_cita(instancia):
    """
    Cancela una cita seleccionada por el usuario.
    
    Permite buscar la cita por ID, nombre, apellido o contacto.
    Si la cita existe y esta pendiente, la marca como cancelada.
...
def eliminar_cita(instancia):
    """
    Elimina una cita seleccionada por el usuario.
    
    Permite buscar la cita por ID, nombre, apellido o contacto.
    Si la cita existe, la elimina permanentemente del sistema.
...
def buscar_por_paciente_interactivo(instancia):
    """
    Busca citas por uno o más criterios del paciente.
    
    Criterios disponibles:
        - nombre: Busca coincidencias parciales en el nombre
        - apellido: Busca en apellido paterno y materno
        - contacto: Busca por número de celular (parcial o completo)
    
    Puede usar cualquier combinación de los campos anteriores.
    Presione Enter en un campo para omitirlo.
    """
    ...
    # Ejecuta la busqueda con los criterios proporcionados
    # Retorna todas las citas que coincidan con TODOS los criterios ingresados
...
def menu_buscar_citas():
    """
    Submenu para buscar citas por diferentes criterios.
    
    Permite al usuario buscar citas por ID, por paciente/contacto
    ...
    while True:
        ...
        match opcion:
            case "1":
                # Busca una cita por su ID unico
                buscar_cita_interactivo(servicio_citas)
            case "2":
                # Busca citas por nombre o numero de contacto del paciente
                buscar_por_paciente_interactivo(servicio_citas)
            case "3":
                # Busca citas para una fecha especifica
                buscar_por_fecha_interactivo(servicio_citas)
            case "4":
                # Sale del submenu y vuelve al menu principal de citas
                break
            case _:
                print("Opcion no valida")
...
# Solicitar hora de inicio con formato 12h
...
def mostrar_horarios_atencion():
    """
    Muestra todos los horarios de atencion configurados.
    """
...
def eliminar_horario_atencion():
    """
    Elimina un horario de atencion seleccionado por el usuario.
    
    Muestra la lista de horarios y permite seleccionar cual eliminar.
    """
...
def menu_horarios_atencion():
    """
    Menu para gestionar los horarios de atencion de la clinica.
    
    Permite agregar, mostrar y eliminar horarios de atencion,
    asi como configurar el descanso.
    """
    while True:
        print("\n=== Horarios de Atencion ===")
        ...
        match opcion:
            ...
            case "2":
                # Muestra todos los horarios configurados
                mostrar_horarios_atencion()
            case "3":
                # Elimina un horario seleccionado
                eliminar_horario_atencion()
            case "4":
                # Sale del submenu y vuelve al menu de citas
                break
            case _:
                print("Opcion no valida")
...
def menu_gestionar_citas():
    """
    Submenu para gestionar las citas: ver historial, completar, cancelar o eliminar.
    
    Permite ver las citas completadas y canceladas, asi como cambiar el estado
    de una cita pendiente (completar/cancelar) o eliminarla permanentemente.
    """
    while True:
        print("\n=== Gestionar Citas ===")
        ...
        match opcion:
            case "1":
                # Muestra todas las citas completadas
                mostrar_citas_completadas(servicio_citas)
            case "2":
                # Muestra todas las citas canceladas
                mostrar_citas_canceladas(servicio_citas)
            case "3":
                # Marca una cita como completada y crea el paciente
...
def menu_citas():
    """
    Menu principal para gestionar las citas medicas.
    
    Permite agendar nuevas citas, mostrar citas, buscar por diferentes
    criterios, ver citas del dia o manana, gestionar horarios de atencion
    y acceder al submenu de gestion de citas.
    """
    while True:
        print("\n=== Menu Citas ===")
        ...
        match opcion:
            ...
            case "3":
                # Abre el submenu de busqueda de citas
                menu_buscar_citas()
            case "4":
                # Muestra las citas programadas para el dia actual
                citas_hoy(servicio_citas)
            case "5":
                # Muestra las citas programadas para el dia de manana
                citas_manana(servicio_citas)
            case "6":
                # Abre el submenu de horarios de atencion
                menu_horarios_atencion()
            case "7":
                # Abre el submenu de gestion de citas
                menu_gestionar_citas()
            case "8":
                # Sale del menu de citas y vuelve al menu principal
                break
            case _:
                print("Opcion no valida")
    ...
    while True:
        ...
        match opcion:
            case "1":
                ...
                if clave == "admin123":   # <-- contraseña fija para simplificar, en un sistema real se deberia manejar de forma segura
                    menu_principal()
                else:
            ...
            case "2":
                ...
                # En un futuro se podria implementar un menu interactivo para pacientes aqui
            case "3":
                print("Hasta pronto!")
                break
            case _:
...
def menu_principal():
    """
    Menu de administracion (el que ya tenia como main()).
    """
    while True:
        print("\n" + "="*60)
